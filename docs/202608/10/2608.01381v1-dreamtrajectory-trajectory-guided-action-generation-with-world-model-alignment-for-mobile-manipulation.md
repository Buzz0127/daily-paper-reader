...
tldr: "移动操作需协调基座与机械臂，动作空间远大于固定基座操作。现有VLA策略直接映射观测到全身动作，缺乏显式任务空间运动规划，且开环执行导致误差累积。DreamTrajectory提出轨迹引导框架：联合预测意图级末端轨迹和全身动作块，并引入轻量级轨迹世界模型，通过测试时搜索-预测-评分选择与规划轨迹最对齐的动作。在MS-HAB上平均成功率从32.3%提升至54.8%，真实任务中达90.0%。"
source: arxiv
...
motivation: 现有VLA策略在移动操作中缺乏显式任务空间运动规划，且开环执行导致计划与实现动作存在差距。
method: 联合预测意图级末端轨迹与动作块，并用轨迹世界模型在测试时搜索-预测-评分以对齐规划轨迹。
result: "MS-HAB成功率从32.3%升至54.8%，真实任务成功率最高达90.0%，接触丰富任务提升最大。"
conclusion: 轨迹引导与测试时对齐有效提升移动操作的协调性与鲁棒性，验证了世界模型在动作生成中的价值。
---

## 摘要
移动操作要求机器人在持续变化的视角和接触条件下协调基座与手臂的运动，其动作空间远大于固定基座操作。现有的视觉-语言-动作（VLA）策略在两个方面存在局限。（i）它们将观测直接映射到全身动作块，在缺乏显式任务空间运动规划的情况下搜索这个大动作空间，使得基座-手臂的协调预测不精确。（ii）它们开环执行预测的动作块，而不检查这些动作是否能够实现策略预期的运动，因此控制误差和未建模的接触会累积为计划运动与实际运动之间的差距。我们提出了DreamTrajectory，一个面向语言条件移动操作的轨迹引导框架，针对每个局限引入了一个组件。针对（i），DreamTrajectory在单个动作专家中联合预测意图级末端执行器轨迹和全身动作块，从而使轨迹显式地引导基座-手臂动作生成，而不是保持隐式。针对（ii），一个轻量级轨迹世界模型预测候选动作块将产生的轨迹，并通过测试时搜索-预测-评分程序选择与计划轨迹最对齐的候选。在MS-HAB上，轨迹引导将平均成功率从32.3%提高到47.5%，测试时细化进一步提高到54.8%，在接触丰富的关节物体任务上提升最大。在三个真实世界移动操作任务上，相应的平均成功率分别为63.3%、81.7%和90.0%。
