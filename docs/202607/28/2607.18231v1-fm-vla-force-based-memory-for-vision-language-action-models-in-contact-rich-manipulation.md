...
tldr: "现有记忆增强VLA模型依赖视觉记忆，在视觉模糊的时序任务中效率低且受限。本文提出FM-VLA，通过变分自编码器将力历史编码为紧凑力记忆令牌，作为额外条件输入动作专家模块。在隐藏方块查找、按钮按压、擦盘子等任务中成功率超80%，推理开销小。该工作为VLA提供了基于力的非马尔可夫时序推理能力，有效利用接触事件历史指导操作。"
source: arxiv
...
motivation: 视觉记忆方法在接触丰富且视觉模糊的时序任务中计算昂贵、效果差，需更高效的时序记忆机制。
method: 用预训练VAE将力历史编码为力记忆令牌，与状态历史共同作为条件输入动作专家模块。
result: "在三个记忆依赖任务中成功率超80%，显著优于基线，推理开销极小。"
conclusion: 力记忆有效补充视觉记忆，为VLA提供鲁棒的接触事件时序推理能力。
---

## 摘要
视觉-语言-动作（VLA）模型在机器人操作中实现了令人印象深刻的泛化能力，最近具有记忆增强的VLA模型通过基于过去的图像或语言摘要进行条件化，放宽了马尔可夫假设。基于视觉的记忆方法通过对采样的过去图像帧进行条件化来解决这一问题，但它们在计算上昂贵，并且当时间事件在视觉上模糊时（例如，用微小动作多次按下按钮）存在根本性限制。我们提出了FM-VLA，一种具有基于力的记忆的VLA模型，能够对非马尔可夫、接触丰富的操作进行时间上下文推理。我们使用预训练了力时间序列重建的变分自编码器（VAE）将力历史编码为紧凑的力记忆令牌。通过将力潜在表示和短期状态历史作为额外的条件令牌投影到动作专家模块，我们使VLA模型能够利用累积的接触事件历史来指导操作。我们在三个依赖记忆的任务上评估了FM-VLA，包括寻找隐藏方块、按下按钮和擦拭盘子特定次数。我们的轻量级力记忆在最小的推理开销下实现了超过80%的成功率，显著优于基线方法。项目页面：https://qft-333.github.io/FM-VLA-Page/
